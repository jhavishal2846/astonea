        ...
        <div className="container-wide">
          <div className="max-w-3xl space-y-12">
            {SECTIONS.map((section, ci) => (
              <SectionBlock key={section.sub} section={section} index={ci} />
            ))}
          </div>
        ...
        <div className="space-y-4">
          ...
          {kmps && kmps.length > 0 && (
            <div className="grid sm:grid-cols-2 gap-3">
              {kmps.map((k, ki) => (
                <Reveal key={k.email} delay={index * 30 + 60 + ki * 40}>
                  <div className="p-4 rounded-lg border" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                    <p className="font-semibold text-sm" style={{ color: 'var(--color-ink)' }}>{k.name}</p>
                    <p className="text-xs mb-2" style={{ color: 'var(--color-ink-muted)' }}>{k.role}</p>
          ...
          {items && items.length > 0 && (
            <div className="space-y-px rounded-lg overflow-hidden" style={{ background: 'var(--color-border)' }}>
              {items.map((item, ii) => {
                ...
                return (
                  <Reveal key={item.title} delay={index * 30 + 40 + ii * 30}>
