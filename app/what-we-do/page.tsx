...
const services = [
  { title: 'Contract Manufacturing', desc: 'End-to-end third-party manufacturing for pharma and cosmetic brands — domestic and international clients.' },
  { title: 'Formulation Development', desc: 'In-house R&D support for new formulations from concept through regulatory clearance.' },
  { title: 'Packaging Material Procurement', desc: 'Sourcing and supply of primary and secondary packaging materials to complement production.' },
  { title: 'Product Export', desc: 'Active exporter to markets including Iraq, Yemen, and beyond. USFDA OTC audit completed.' },
  { title: 'Quality Assurance', desc: 'Multi-tier QMS with in-process checks, stability studies, and NABL-accredited third-party audits.' },
  { title: 'Regulatory Affairs', desc: 'Complete dossier preparation, labelling compliance, and export documentation support.' },
]
    ...
    <div className="flex-1 flex flex-col">
      ...
      <section className="py-24 lg:py-32" style={{ background: 'var(--color-bg)' }}>
        <div className="container-wide">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-px rounded-2xl overflow-hidden" style={{ background: 'var(--color-border)' }}>
            {services.map((s) => (
              <div key={s.title} className="p-8" style={{ background: 'var(--color-surface)' }}>
                <h3 className="font-display text-lg font-semibold mb-3" style={{ color: 'var(--color-ink)' }}>{s.title}</h3>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {pharmaCategories.map((c, i) => (
              <Reveal key={c.label} delay={i * 50}>
                <div className="p-6 rounded-xl border" style={{ background: 'rgba(255,255,255,0.04)', borderColor: 'rgba(255,255,255,0.08)' }}>
                  <h3 className="font-semibold text-white mb-2">{c.label}</h3>
                  <p className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.45)' }}>{c.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
                ...
                <div className="p-8 rounded-2xl border h-full flex flex-col" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <span className="text-xs font-medium px-2.5 py-1 rounded-full" style={{
                      ...
                      color: b.status === 'Live' ? 'var(--color-primary)' : 'var(--color-accent-dark)',
                    }}>
                      {b.status}
                    </span>
                  </div>
        ...
        <div className="container-wide">
          <StaggerReveal className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
            <div>
              ...
              <p className="mt-3 text-sm text-white/60 max-w-md">Our team is ready to discuss your requirements and bring your product to market.</p>
            </div>
            <div className="flex flex-wrap gap-3">
              ...
              <Link href="/manufacturing-facility" className="inline-flex items-center px-6 py-3 rounded-full border border-white/30 text-white/80 text-sm font-medium hover:border-white/60 hover:text-white transition-colors">
                View Facility
              </Link>
            </div>
          </StaggerReveal>
