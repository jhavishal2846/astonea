...
export default function HerbalExtractsPage() {
  const [search, setSearch] = useState('')
  const [page, setPage]     = useState(1)

  const filtered = extracts.filter(e =>
    e.name.toLowerCase().includes(search.toLowerCase()) || e.cas.includes(search)
  )
  ...
  return (
    <>
      <ProductPageHeader
        ...
        subtitle="Standardised botanical extracts with guaranteed active content, validated by HPLC. Ayurvedic, adaptogenic, and global botanical sources."
        breadcrumbs={[{ label: 'Products', href: '/products' }, { label: 'Herbal Extracts' }]}
        variant="dark"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-surface-raised">
                    <th className="text-left px-6 py-4 text-xs uppercase tracking-widest text-ink-subtle font-semibold">Botanical Name</th>
                    <th className="text-left px-6 py-4 text-xs uppercase tracking-widest text-ink-subtle font-semibold">Standardisation</th>
