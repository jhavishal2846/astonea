...
export default async function CSRPage() {
  const t = await getPageText('/csr')
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Corporate Social Responsibility') as string}
        title={t('header.title', 'CSR') as string}
        description={t('header.description', 'Sowing responsibility today, reaping a brighter world tomorrow.') as string}
            ...
            <Reveal delay={100}>
              <div className="p-10 rounded-3xl" style={{ background: 'var(--color-slate-950)' }}>
                <p className="font-display text-2xl font-semibold italic text-white leading-snug mb-6">
                  &ldquo;{t('csr.philosophy.quote', 'Sowing responsibility today, reaping a brighter world tomorrow.')}&rdquo;
                </p>
                <p className="text-sm" style={{ color: 'rgba(255,255,255,0.72)' }}>
                  {t('csr.philosophy.quote_attribution', '— Astonea Labs CSR Philosophy')}
                </p>
              </div>
            </Reveal>
          </div>
              ...
              <Reveal key={i} delay={i * 80}>
                <div className="flex flex-col gap-4 p-10 h-full" style={{ background: 'var(--color-bg)' }}>
                  <span className="font-display text-4xl font-bold tracking-tighter leading-none select-none" style={{ color: 'var(--color-primary-light)' }}>
                    {t(`csr.focus_${i}.num`, f.num)}
                  </span>
                  <h3 className="font-display text-xl font-semibold" style={{ color: 'var(--color-ink)' }}>{t(`csr.focus_${i}.title`, f.title)}</h3>
                  <p className="text-sm leading-relaxed" style={{ color: 'var(--color-ink-muted)' }}>{t(`csr.focus_${i}.desc`, f.desc)}</p>
                </div>
              </Reveal>
            ))}
          </div>
          ...
          <div className="max-w-3xl">
              ...
              {principles.map((p, i) => (
                <Reveal key={i} delay={i * 60}>
                  <div className="flex items-start gap-5 p-6" style={{ background: 'var(--color-surface)' }}>
                    ...
                    <p className="text-sm leading-relaxed pt-0.5" style={{ color: 'var(--color-ink-muted)' }}>{t(`csr.principle_${i}`, p)}</p>
                  </div>
                </Reveal>
              ))}
            </div>
            ...
            <Reveal delay={200}>
              <div className="mt-10 flex flex-wrap items-center gap-4">
                ...
                >
                  {t('csr.cta.primary', 'View CSR Policy')}
                </a>
                <a
                  href="/annual-reports"
                ...
                >
                  {t('csr.cta.secondary', 'View Annual Reports')}
                </a>
              </div>
