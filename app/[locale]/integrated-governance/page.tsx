...
export default async function IntegratedGovernancePage() {
  ...
  const t = await getPageText('/integrated-governance')

  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Integrated Filings') as string}
        title={t('header.title', 'Integrated Governance') as string}
        description={t('header.description', 'Quarterly integrated governance filings of Astonea Labs Limited submitted under the SEBI integrated filing framework.') as string}
        breadcrumb={[
      ...
      <section className="py-14 lg:py-12" style={{ background: 'var(--color-bg)' }}>
        <div className="container-wide">
          <div className="max-w-3xl">
            <DocumentList items={items} emptyMessage={t('doc.empty', 'No integrated governance filings have been published yet.') as string} />
          </div>
        </div>
      </section>
    </div>
