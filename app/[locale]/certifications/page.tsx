...
export default async function CertificationsPage() {
  const t = await getPageText('/certifications')
  ...
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Quality') as string}
        title={t('header.title', 'Certifications') as string}
        description={t('header.description', 'Our quality credentials and regulatory approvals — validating our commitment to manufacturing excellence.') as string}
                ...
                <div className="flex flex-col p-8 rounded-2xl border h-full" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                  ...
                  {c.period && (
                    <div className="pt-4 border-t" style={{ borderColor: 'var(--color-border)' }}>
                      <span className="text-xs font-medium" style={{ color: 'var(--color-ink-subtle)' }}>{t('cert.scope_label', 'Scope:')} {c.period}</span>
                    </div>
                  )}
                </div>
              </Reveal>
                ...
                {qualityPractices.map((p, i) => (
                  <div key={i} className="px-6 py-4" style={{ background: 'var(--color-slate-950)' }}>
                    <div className="pl-4 border-l-2" style={{ borderColor: 'var(--color-primary-light)' }}>
                      <span className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.78)' }}>{t(`cert.practice_${i}`, p)}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Reveal>
            <div>
              <h2 className="font-display text-2xl font-bold mb-2" style={{ color: 'var(--color-ink)' }}>
                {t('cert.cta.heading', 'Need compliance documentation?')}
              </h2>
              <p className="text-sm" style={{ color: 'var(--color-ink-muted)' }}>
                {t('cert.cta.body', 'Contact our team for GMP certificates, audit reports, and regulatory documents.')}
              </p>
            </div>
          </Reveal>
          <Reveal delay={80}>
            <div className="flex flex-wrap gap-3">
              {downloadHref && (
                <a href={downloadHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-sm font-bold border transition-colors hover:bg-blue-50" style={{ borderColor: 'var(--color-primary)', color: 'var(--color-primary)' }}>
                  {t('cert.cta.primary', 'Download Certificates')}
                </a>
              )}
              <Link href="/contact-us" className="inline-flex items-center px-6 py-3 rounded-full text-white text-sm font-bold transition-colors hover:opacity-90" style={{ background: 'var(--color-primary)' }}>
                {t('cert.cta.secondary', 'Contact Our QA Team')}
              </Link>
            </div>
          </Reveal>
        </div>
