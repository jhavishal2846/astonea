...
export default async function VisionAndMissionPage() {
  const t = await getPageText('/vision-and-mission')
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Our Purpose') as string}
        title={t('header.title', 'Vision & Mission') as string}
        description={t('header.description', 'The beliefs and ambitions that guide every decision at Astonea Labs Limited.') as string}
            ...
            <Reveal>
              <div className="p-10 rounded-3xl" style={{ background: 'var(--color-primary)' }}>
                <p className="text-xs font-semibold uppercase tracking-widest mb-4" style={{ color: 'rgba(255,255,255,0.72)' }}>
                  {t('vision.label', 'Our Vision')}
                </p>
                <blockquote className="font-display text-2xl lg:text-3xl font-semibold leading-snug text-white text-balance">
                  &ldquo;{t('vision.quote', "To build sustainable brands that have global reach and make a lasting impact on people's life.")}&rdquo;
                </blockquote>
            ...
            <Reveal delay={100}>
              <div className="lg:pt-2">
                <p className="text-xs font-semibold uppercase tracking-widest mb-4" style={{ color: 'var(--color-primary)' }}>
                  {t('mission.label', 'Our Mission')}
                </p>
                <blockquote className="font-display text-2xl lg:text-3xl font-semibold leading-snug text-balance" style={{ color: 'var(--color-ink)' }}>
                  &ldquo;{t('mission.quote', 'To reach our wings across the globe with our own brands and also be a preferred supplier globally.')}&rdquo;
                </blockquote>
              ...
              <Reveal key={i} delay={i * 60}>
                <div className="flex flex-col gap-4 p-8 h-full" style={{ background: 'var(--color-bg)' }}>
                  <span className="font-display text-4xl font-bold tracking-tighter leading-none select-none" style={{ color: 'var(--color-primary-light)' }}>
                    {t(`value_${i}.num`, v.num)}
                  </span>
                  <h3 className="font-display text-lg font-semibold" style={{ color: 'var(--color-ink)' }}>{t(`value_${i}.title`, v.title)}</h3>
                  <p className="text-sm leading-relaxed" style={{ color: 'var(--color-ink-muted)' }}>{t(`value_${i}.desc`, v.desc)}</p>
                </div>
              </Reveal>
            ))}
          </div>
