...
export default async function WhatWeDoPage() {
  const t = await getPageText('/what-we-do')
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Capabilities') as string}
        title={t('header.title', 'What We Do') as string}
        description={t('header.description', 'A diversified, innovation-driven enterprise specialising in the manufacturing, marketing, and global distribution of pharmaceutical and cosmetic products.') as string}
        ...
        <div className="container-wide">
          <div className="grid grid-cols-2 lg:grid-cols-4">
            {metrics.map((m, i) => (
              ...
              >
                <p className="font-display text-3xl font-bold text-white mb-1">{t(`wwd.metric_${i}.value`, m.value)}</p>
                <p className="text-xs font-medium uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.72)' }}>{t(`wwd.metric_${i}.label`, m.label)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-14 lg:py-12" style={{ background: 'var(--color-bg)' }}>
        <div className="container-wide">
          <Reveal>
            <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--color-primary)' }}>{t('wwd.services.label', 'Services')}</p>
            <h2 className="font-display text-3xl lg:text-4xl font-bold leading-snug mb-14 text-balance" style={{ color: 'var(--color-ink)' }}>
              {t('wwd.services.heading', 'End-to-end manufacturing — tailored to your brand')}
            </h2>
          </Reveal>
              ...
              <Reveal key={i} delay={i * 50}>
                <div className="flex flex-col gap-3 p-8 h-full" style={{ background: 'var(--color-surface)' }}>
                  <span className="font-display text-4xl font-bold tracking-tighter leading-none select-none" style={{ color: 'var(--color-primary-light)' }}>
                    {t(`wwd.service_${i}.num`, s.num)}
                  </span>
                  <h3 className="font-display text-lg font-semibold" style={{ color: 'var(--color-ink)' }}>{t(`wwd.service_${i}.title`, s.title)}</h3>
                  <p className="text-sm leading-relaxed" style={{ color: 'var(--color-ink-muted)' }}>{t(`wwd.service_${i}.desc`, s.desc)}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="py-14 lg:py-12" style={{ background: 'var(--color-slate-950)' }}>
        <div className="container-wide">
          <Reveal>
            <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--color-primary-light)' }}>{t('wwd.pharma.label', 'Pharmaceutical')}</p>
            <h2 className="font-display text-3xl lg:text-4xl font-bold leading-snug mb-4 text-white text-balance">
              {t('wwd.pharma.heading', 'Therapeutic formulations across every major category')}
            </h2>
            ...
            {pharmaCategories.map((c, i) => (
              <Reveal key={i} delay={i * 50}>
                <div className="p-6 rounded-xl border" style={{ background: 'rgba(255,255,255,0.05)', borderColor: 'rgba(255,255,255,0.1)' }}>
                  <h3 className="font-semibold text-white mb-2">{t(`wwd.pcat_${i}.label`, c.label)}</h3>
                  <p className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.72)' }}>{t(`wwd.pcat_${i}.desc`, c.desc)}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="py-14 lg:py-12" style={{ background: 'var(--color-surface)' }}>
        <div className="container-wide">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <Reveal>
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--color-primary)' }}>{t('wwd.cosmetics.label', 'Cosmetics & Personal Care')}</p>
                <h2 className="font-display text-3xl lg:text-4xl font-bold leading-snug mb-6 text-balance" style={{ color: 'var(--color-ink)' }}>
                  {t('wwd.cosmetics.heading', 'Beauty and personal care with safety at the core')}
                </h2>
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {cosmeticFormats.map((f, i) => (
                    <span key={i} className="px-3 py-1.5 rounded-full text-xs font-medium border" style={{ background: 'var(--color-bg)', borderColor: 'var(--color-border)', color: 'var(--color-ink-muted)' }}>
                      {t(`wwd.cformat_${i}`, f)}
                    </span>
                  ))}
                </div>
            ...
            <StaggerReveal className="space-y-4">
              {cosmeticCategories.map((c, i) => (
                <div key={i} className="p-6 rounded-xl border" style={{ background: 'var(--color-bg)', borderColor: 'var(--color-border)' }}>
                  <h3 className="font-semibold mb-2" style={{ color: 'var(--color-ink)' }}>{t(`wwd.ccat_${i}.label`, c.label)}</h3>
                  <p className="text-sm leading-relaxed" style={{ color: 'var(--color-ink-muted)' }}>{t(`wwd.ccat_${i}.desc`, c.desc)}</p>
                </div>
              ))}
            </StaggerReveal>
          </div>
      ...
      <section className="py-14 lg:py-12" style={{ background: 'var(--color-bg)' }}>
        <div className="container-wide">
          <Reveal>
            <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--color-primary)' }}>{t('wwd.brands.label', 'Brand Portfolio')}</p>
            <h2 className="font-display text-3xl lg:text-4xl font-bold leading-snug mb-14 text-balance" style={{ color: 'var(--color-ink)' }}>
              {t('wwd.brands.heading', 'Our own consumer brands')}
            </h2>
          </Reveal>
              ...
              <Reveal key={i} delay={i * 80}>
                <div className="p-8 rounded-2xl border h-full flex flex-col" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                  <div className="flex items-start justify-between mb-4">
                    <h3 className="font-display text-xl font-semibold" style={{ color: 'var(--color-ink)' }}>{t(`wwd.brand_${i}.name`, b.name)}</h3>
                    <span className="text-xs font-medium px-2.5 py-1 rounded-full" style={{
                      ...
                      {t(`wwd.brand_${i}.status`, b.status)}
                    </span>
                  </div>
                  <p className="text-sm leading-relaxed flex-1" style={{ color: 'var(--color-ink-muted)' }}>{t(`wwd.brand_${i}.desc`, b.desc)}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="py-20" style={{ background: 'var(--color-primary)' }}>
        <div className="container-wide">
          <StaggerReveal className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
            <div>
              <h2 className="font-display text-3xl font-bold text-white">{t('wwd.cta.heading', 'Have a formulation in mind?')}</h2>
              <p className="mt-3 text-sm max-w-md" style={{ color: 'rgba(255,255,255,0.78)' }}>{t('wwd.cta.body', 'Our team is ready to discuss your requirements and bring your product to market.')}</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link href="/contact-us" className="inline-flex items-center px-6 py-3 rounded-full bg-white text-sm font-bold transition-colors hover:bg-slate-100" style={{ color: 'var(--color-primary)' }}>
                {t('wwd.cta.primary', 'Start a Conversation')}
              </Link>
              <Link href="/manufacturing-facility" className="inline-flex items-center px-6 py-3 rounded-full border text-sm font-medium transition-colors hover:bg-white/10" style={{ borderColor: 'rgba(255,255,255,0.4)', color: 'rgba(255,255,255,0.88)' }}>
                {t('wwd.cta.secondary', 'View Facility')}
              </Link>
            </div>
          </StaggerReveal>
        </div>
