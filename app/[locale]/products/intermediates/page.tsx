...
import { intermediatesListing as products } from '@/lib/products/listings'
...
export default function IntermediatesPage() {
  ...
  return (
    <>
      <ProductPageHeader
        title="Intermediates"
        subtitle="High-purity synthesis intermediates for pharmaceutical, agrochemical, and fine chemical manufacturing worldwide."
        breadcrumbs={[{ label: 'Products', href: '/products' }, { label: 'Intermediates' }]}
        variant="dark"
        tag="Synthesis Intermediates"
      />
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-surface-raised">
                    ...
                    <th className="text-left px-6 py-4 text-xs uppercase tracking-widest text-ink-subtle font-semibold">Purity</th>
                  ...
                  <motion.tbody key={page} variants={listVariants} initial="hidden" animate="show" exit="exit">
                    {paged.map(p => (
                      <motion.tr
                        ...
                        onClick={() => router.push(`/products/intermediates/${toSlug(p.name)}`)}
                        onKeyDown={(e) => { if (e.key === 'Enter') router.push(`/products/intermediates/${toSlug(p.name)}`) }}
                        tabIndex={0}
                        role="link"
                      ...
                      >
                        ...
                        <td className="px-6 py-4"><span className="px-2.5 py-0.5 text-xs font-semibold bg-primary-xlight text-primary rounded-full">{p.purity}</span></td>
