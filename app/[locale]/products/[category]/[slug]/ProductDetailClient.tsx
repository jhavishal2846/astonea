...
/**
 * Per-document-type colour + catalog-key suffix. `/` isn't valid in i18n
 * message keys, so 'CEP/COS' and 'WHO-GMP' are mapped to underscored
 * variants for the lookup; the label itself is fetched via
 * `products.doc.<keySuffix>`.
 */
const docConfig: Record<DocumentType, { color: string; keySuffix: string }> = {
  'COA':     { color: '#0072CE', keySuffix: 'COA' },
  'MSDS':    { color: '#10B981', keySuffix: 'MSDS' },
  'DMF':     { color: '#6366F1', keySuffix: 'DMF' },
  'CEP/COS': { color: '#F97316', keySuffix: 'CEP_COS' },
  'TDS':     { color: '#0F766E', keySuffix: 'TDS' },
  'COPP':    { color: '#84CC16', keySuffix: 'COPP' },
  'FSSAI':   { color: '#EF4444', keySuffix: 'FSSAI' },
  'WHO-GMP': { color: '#E8A900', keySuffix: 'WHO_GMP' },
}
...
interface Props {
  ...
  /** Admin-editable label from the DB. Falls back to the hardcoded map. */
...
export default function ProductDetailClient({ product, category, siblings, categoryLabelOverride }: Props) {
  const t = useTranslations()
  const catLabel = categoryLabelOverride ?? categoryLabel(category)
  ...
  return (
    <>
      <ProductPageHeader
        title={product.name}
        subtitle={t('products.detail.subtitle', { cas: product.cas, grade: product.grade, category: product.category })}
        breadcrumbs={[
        ...
        <div className="container-wide">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <motion.div variants={fadeUp} initial="hidden" animate="show"
                ...
                <h2 className="font-display font-bold text-xl text-ink mb-4">{t('products.detail.overview')}</h2>
              ...
              <motion.div variants={fadeUp} initial="hidden" animate="show"
                ...
                className="bg-surface border border-border rounded-2xl p-8">
                <h2 className="font-display font-bold text-xl text-ink mb-5">{t('products.detail.applications')}</h2>
              ...
              {hasSpecs && (
                <motion.div variants={fadeUp} initial="hidden" animate="show"
                  ...
                  className="bg-surface border border-border rounded-2xl p-8">
                  <h2 className="font-display font-bold text-xl text-ink mb-5">{t('products.detail.specs')}</h2>
                  <div>
                    <SpecRow label={t('products.detail.spec.cas')} value={product.cas} />
                    <SpecRow label={t('products.detail.spec.grade')} value={product.grade} />
                    <SpecRow label={t('products.detail.spec.formula')} value={product.molecularFormula} />
                    <SpecRow label={t('products.detail.spec.weight')} value={product.molecularWeight} />
                    <SpecRow label={t('products.detail.spec.purity')} value={product.purity} />
                    <SpecRow label={t('products.detail.spec.appearance')} value={product.appearance} />
                    <SpecRow label={t('products.detail.spec.storage')} value={product.storageConditions} />
                  </div>
                </motion.div>
              )}
              ...
              <motion.div variants={fadeUp} initial="hidden" animate="show"
                ...
                className="bg-surface border border-border rounded-2xl p-8">
                <h2 className="font-display font-bold text-xl text-ink mb-5">{t('products.detail.packaging')}</h2>
                ...
                <p className="text-xs text-ink-subtle mt-4">{t('products.detail.packaging_note')}</p>
              </motion.div>
            </div>
            ...
            <div className="space-y-6 lg:sticky lg:top-28 lg:self-start">
              ...
              <motion.div variants={fadeUp} initial="hidden" animate="show"
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-3 pb-5 mb-5 border-b border-white/15">
                  {[
                    { label: t('products.detail.spec.cas'), value: product.cas },
                    { label: t('products.detail.spec.grade'), value: product.grade },
                    { label: t('products.detail.spec.category'), value: product.category },
                    ...(product.purity ? [{ label: t('products.detail.spec.purity'), value: product.purity }] : []),
                  ].map(({ label, value }) => (
                    <div key={label}>
                ...
                <h3 className="font-display font-bold text-lg mb-1.5">{t('products.detail.quote.heading')}</h3>
                <p className="text-white/70 text-sm mb-5 leading-relaxed">
                  {t('products.detail.quote.subtext')}
                </p>
                <Link
                ...
                >
                  {t('products.detail.quote.enquire')}
                </Link>
                <Link
                  href="/contact-us"
                ...
                >
                  {t('products.detail.quote.expert')}
                </Link>
                ...
                <div className="mt-5 pt-5 border-t border-white/15 flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/55">{t('products.detail.quote.response')}</span>
                </div>
              </motion.div>
              ...
              <motion.div variants={fadeUp} initial="hidden" animate="show"
                ...
                <h3 className="font-display font-bold text-base text-ink mb-4">{t('products.detail.documentation')}</h3>
                    ...
                    return (
                      <div key={doc} className="flex items-center gap-3 p-3 rounded-xl border border-border hover:border-primary/20 hover:bg-primary-xlight/20 transition-all duration-200 cursor-pointer group">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-ink-subtle truncate">{t(`products.doc.${cfg.keySuffix}`)}</div>
                        </div>
                ...
                <p className="text-xs text-ink-subtle mt-3">{t('products.detail.docs_note')}</p>
              </motion.div>
            </div>
          </div>
          ...
          {siblings.length > 0 && (
            <motion.div
              ...
              className="mt-12"
            >
              <h2 className="font-display font-bold text-2xl text-ink mb-6">{t('products.detail.related')}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                {siblings.map((sib) => (
                  ...
                  >
                    ...
                    <p className="text-xs font-mono text-ink-subtle">{t('products.detail.cas_prefix')} {sib.cas}</p>
                  </Link>
                ))}
              </div>
            </motion.div>
          ...
          <div className="mt-10 pt-8 border-t border-border flex items-center justify-between">
            ...
            >
              ...
              {t('products.detail.back_to', { label: catLabel })}
            </Link>
            <Link
            ...
            >
              {t('products.detail.quote.heading')}
