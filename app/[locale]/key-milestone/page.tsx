...
export default async function KeyMilestonePage() {
  const t = await getPageText('/key-milestone')
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Our Journey') as string}
        title={t('header.title', 'Key Milestones') as string}
        description={t('header.description', 'From a Chandigarh startup in 2017 to a BSE SME–listed, pan-India manufacturer in 2025.') as string}
        breadcrumb={[{ label: 'Key Milestones', href: '/key-milestone' }]}
      />

      <MilestoneStats stats={stats} keyPrefix="km.stat" />
      <MilestoneTimeline milestones={milestones} keyPrefix="km.ms" />
    </div>
  )
}
