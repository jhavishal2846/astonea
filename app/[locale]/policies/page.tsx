...
export default async function PoliciesPage() {
  ...
  const t = await getPageText('/policies')

  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Governance') as string}
        title={t('header.title', 'Policies') as string}
        description={t('header.description', 'Board-approved governance policies of Astonea Labs Limited.') as string}
        breadcrumb={[
            ...
            {items.length === 0 ? (
              <Reveal>
                <div className="p-6 rounded-xl border text-sm" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)', color: 'var(--color-ink-muted)' }}>
                  {t('doc.empty', 'No policies have been published yet.')}
                </div>
              </Reveal>
            ) : (
