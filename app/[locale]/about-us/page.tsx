...
export default async function AboutUsPage() {
  ...
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Our Company') as string}
        title={t('header.title', 'Inspiring Trust in Healthcare, Elevating Beauty with Elegance') as string}
        description={t('header.description', 'Astonea Labs Limited is a GMP-certified, BSE-SME listed pharmaceutical and cosmetic contract manufacturer headquartered in Chandigarh — delivering quality, regulatory confidence, and supply-chain reliability since 2017.') as string}
        ...
        <div className="container-wide">
          <div className="grid grid-cols-2 lg:grid-cols-4">
            {trustMetrics.map((m, i) => {
              ...
              return (
                <div key={i} className={`flex flex-col items-center py-10 px-6 text-center ${borderClass}`}>
                  <span className="font-display text-4xl lg:text-5xl font-bold text-white tracking-tight">{t(`about.metric_${i}.value`, m.value)}</span>
                  <span className="mt-1.5 text-xs font-semibold uppercase tracking-widest text-white">{t(`about.metric_${i}.label`, m.label)}</span>
                  <span className="mt-0.5 text-xs" style={{ color: 'rgba(255,255,255,0.72)' }}>{t(`about.metric_${i}.sub`, m.sub)}</span>
                </div>
              )
            })}
          </div>
          ...
          <Reveal>
            <div className="max-w-4xl mx-auto text-center">
              <p className="text-xs font-semibold uppercase tracking-widest mb-6" style={{ color: 'var(--color-primary)' }}>
                {t('about.who_we_are.label', 'Who We Are')}
              </p>
              <h2 className="font-display text-4xl lg:text-6xl font-bold leading-tight tracking-tight text-balance" style={{ color: 'var(--color-ink)' }}>
                {t('about.who_we_are.heading', "India's trusted name in third-party pharmaceutical & cosmetic manufacturing")}
              </h2>
        ...
        <div className="container-wide">
          <Reveal>
            <p className="text-xs font-semibold uppercase tracking-widest text-center mb-10" style={{ color: 'var(--color-ink-subtle)' }}>
              {t('about.certifications.label', 'Regulatory & Compliance Credentials')}
            </p>
          </Reveal>
            ...
            {certifications.map((c, i) => (
              <Reveal key={i} delay={i * 60}>
                ...
                >
                  ...
                  >
                    {t(`about.cert_${i}.code`, c.code)}
                  </div>
                  <div>
                    <p className="text-sm font-semibold" style={{ color: 'var(--color-ink)' }}>{t(`about.cert_${i}.label`, c.label)}</p>
                    <p className="mt-1 text-xs leading-relaxed" style={{ color: 'var(--color-ink-subtle)' }}>{t(`about.cert_${i}.desc`, c.desc)}</p>
                  </div>
                </div>
              </Reveal>
            ))}
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-start">
            <StaggerReveal>
              <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--color-primary)' }}>
                {t('about.foundation.label', 'Our Foundation')}
              </p>
              <h2 className="font-display text-3xl lg:text-4xl font-bold leading-snug tracking-tight text-balance" style={{ color: 'var(--color-ink)' }}>
                {t('about.foundation.heading', 'GMP-compliant, BSE-SME, and built for scale')}
              </h2>
              <p className="text-base leading-relaxed" style={{ color: 'var(--color-ink-muted)' }}>
                {t(
                  'about.foundation.body_1',
            ...
            <StaggerReveal className="space-y-4">
              {[
                ...
                { label: 'Market Status', value: 'BSE-SME Limited Company' },
            ...
            {pathCards.map((card, i) => (
              <Reveal key={i} delay={i * 80}>
                ...
                >
                  ...
                  >
                    {t('about.path_kicker', 'I am a')}
                  </p>
                  <h3 className="font-display text-xl font-bold text-white leading-snug">{t(`about.path_${i}.role`, card.role)}</h3>
                  <p className="text-sm leading-relaxed flex-1" style={{ color: 'rgba(255,255,255,0.72)' }}>
                    {t(`about.path_${i}.desc`, card.desc)}
                  </p>
        ...
        <div className="container-wide">
          <div className="grid lg:grid-cols-5 gap-16 lg:gap-20 items-start">
            <Reveal className="lg:col-span-3">
              <p className="text-xs font-semibold uppercase tracking-widest mb-6" style={{ color: 'var(--color-primary)' }}>
                {t('about.founder.label', 'Founder & Managing Director')}
              </p>
              <blockquote
              ...
              <div className="flex items-center gap-4">
                <div
                  ...
                  aria-hidden="true"
                >
                  {t('about.founder.initials', 'AG')}
                </div>
                <div>
                  <p className="font-semibold text-sm" style={{ color: 'var(--color-ink)' }}>{t('about.founder.name', 'Mr. Ashish Gulati')}</p>
                  <p className="text-xs mt-0.5" style={{ color: 'var(--color-ink-subtle)' }}>
                    {t('about.founder.title', 'Founder & Managing Director, Astonea Labs Limited')}
                  </p>
                </div>
              </div>
            </Reveal>
            ...
            <Reveal delay={120} className="lg:col-span-2">
              ...
              >
                ...
                >
                  {t('about.founder.cta', 'Meet the Full Leadership Team')}
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-16">
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--color-primary)' }}>
                  {t('about.pillars.label', 'Our Pillars')}
                </p>
                <h2 className="font-display text-3xl lg:text-4xl font-bold leading-snug tracking-tight text-balance" style={{ color: 'var(--color-ink)' }}>
                  {t('about.pillars.heading', 'What drives everything we do')}
                </h2>
              </div>
              <p className="text-sm max-w-xs leading-relaxed lg:text-right" style={{ color: 'var(--color-ink-muted)' }}>
                {t(
                  'about.pillars.subtext',
              ...
              <Reveal key={i} delay={i * 80}>
                ...
                >
                  <span
                    ...
                    style={{ color: 'var(--color-primary-light)' }}
                  >
                    {t(`about.pillar_${i}.num`, p.num)}
                  </span>
                  <div className="flex flex-col gap-3 flex-1">
                    <h3 className="font-display text-xl font-semibold" style={{ color: 'var(--color-ink)' }}>{t(`about.pillar_${i}.title`, p.title)}</h3>
                    <p className="text-sm leading-relaxed" style={{ color: 'var(--color-ink-muted)' }}>{t(`about.pillar_${i}.desc`, p.desc)}</p>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      <section className="py-14 lg:py-12" style={{ background: 'var(--color-slate-950)' }}>
        <div className="container-wide">
          <Reveal>
            <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--color-primary-light)' }}>
              {t('about.sectors.label', 'Sectors We Serve')}
            </p>
            <h2 className="font-display text-3xl lg:text-4xl font-bold text-white leading-snug mb-16 text-balance">
              {t('about.sectors.heading', 'Dual-sector expertise, one trusted partner')}
            </h2>
          </Reveal>
            ...
            <div className="grid sm:grid-cols-2 gap-8">
              <Reveal>
                <div>
                  ...
                  >
                    {t('about.pharma.tag', 'Pharmaceutical')}
                  </span>
                  <ul className="space-y-3">
                    {pharmaItems.map((item, i) => (
                      <li key={i} className="flex items-center gap-3 text-sm" style={{ color: 'rgba(255,255,255,0.82)' }}>
                        <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: 'var(--color-primary-light)' }} />
                        {t(`about.pharma_${i}`, item)}
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <Reveal delay={120}>
              ...
              >
                <p className="text-xs font-semibold uppercase tracking-widest mb-6" style={{ color: 'var(--color-primary-light)' }}>
                  {t('about.why.label', 'Why Clients Choose Us')}
                </p>
                <ul className="space-y-4">
                  {[
                    ...
                    <li key={i} className="flex items-start gap-3">
                      ...
                      <span className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.82)' }}>{t(`about.why_${i}`, d)}</span>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <StaggerReveal className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-10">
            <div className="max-w-xl">
              <h2 className="font-display text-3xl lg:text-4xl font-bold text-white leading-snug">
                {t('about.cta.heading', 'Ready to bring your product to market?')}
              </h2>
            ...
            <div className="flex flex-wrap gap-3 flex-shrink-0">
              <Link
                ...
                style={{ color: 'var(--color-primary)' }}
              >
                {t('about.cta.primary', 'Get in Touch')}
              ...
              >
                {t('about.cta.secondary', 'Our Services')}
              </Link>
            </div>
          </StaggerReveal>
        </div>
