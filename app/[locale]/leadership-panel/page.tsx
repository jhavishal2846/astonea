...
function PeopleList({ people, keyBase }: { people: Person[]; keyBase: string }) {
  return (
    <div className="mx-auto max-w-5xl space-y-14 lg:space-y-20">
      {people.map((p, i) => (
        <AnimatedPersonRow key={i} person={p} reverse={i % 2 === 1} keyPrefix={`${keyBase}_${i}`} />
      ))}
    </div>
  )
}

export default async function LeadershipPanelPage() {
  const t = await getPageText('/leadership-panel')
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'People') as string}
        title={t('header.title', 'Leadership Panel') as string}
        description={t('header.description', "The experienced team of Directors, KMP's & SM driving Astonea Labs forward.") as string}
      ...
      <section className="py-20 lg:py-28" style={{ background: 'var(--color-bg)' }}>
        <div className="container-wide">
          <div className="mb-14 lg:mb-20">
            <SectionHeader title={t('lp.bm.heading', 'Board of Directors')} />
          </div>
          <PeopleList people={boardMembers} keyBase="lp.bm" />
        </div>
      </section>
      ...
      <section className="py-20 lg:py-28" style={{ background: 'var(--color-surface)' }}>
        <div className="container-wide">
          <div className="mb-14 lg:mb-20">
            <SectionHeader title={t('lp.kmp.heading', 'Key Managerial Personnel')} />
          </div>
          <PeopleList people={kmp} keyBase="lp.kmp" />
        </div>
      </section>
      ...
      <section className="py-20 lg:py-28" style={{ background: 'var(--color-bg)' }}>
        <div className="container-wide">
          <div className="mb-14 lg:mb-20">
            <SectionHeader title={t('lp.sm.heading', 'Senior Managerial Personnel')} />
          </div>
          <PeopleList people={seniorManagement} keyBase="lp.sm" />
        </div>
      </section>
    </div>
  )
