...
export function AnimatedPersonRow({
  person,
  reverse,
  keyPrefix,
}: {
  ...
  reverse: boolean
  /**
   * Optional CMS key prefix. When supplied, name/title/bio are looked up via
   * `usePageText()` as `${keyPrefix}.name`, `${keyPrefix}.title`, `${keyPrefix}.bio`,
   * so admins can override them per-person and edit them in-place.
   */
  keyPrefix?: string
}) {
  ...
  const name = keyPrefix ? t(`${keyPrefix}.name`, person.name) : person.name
  const title = keyPrefix ? t(`${keyPrefix}.title`, person.title) : person.title
  const bio = keyPrefix ? t(`${keyPrefix}.bio`, person.bio) : person.bio
