...
export default async function CorporateGovernancePage() {
  const t = await getPageText('/corporate-governance')
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Investor Relations') as string}
        title={t('header.title', 'Corporate Governance') as string}
        description={t('header.description', "Astonea Labs Limited's governance structure — transparent, accountable, and fully compliant with SEBI Listing Obligations.") as string}
        breadcrumb={[
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            ...
            <Reveal delay={100}>
              <div className="space-y-px" style={{ background: 'var(--color-border)' }}>
                {governanceFacts.map((f, i) => (
                  <div key={i} className="p-5" style={{ background: 'var(--color-surface)' }}>
                    <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: 'var(--color-ink-subtle)' }}>{t(`gov.fact_${i}.label`, f.label)}</p>
                    <p className="font-display text-lg font-semibold mb-0.5" style={{ color: 'var(--color-ink)' }}>{t(`gov.fact_${i}.value`, f.value)}</p>
                    <p className="text-xs" style={{ color: 'var(--color-ink-muted)' }}>{t(`gov.fact_${i}.note`, f.note)}</p>
                  </div>
                ))}
              </div>
            </Reveal>
              ...
              <Reveal key={i} delay={i * 60}>
                <div className="flex flex-col gap-3 p-8 h-full" style={{ background: 'var(--color-bg)' }}>
                  <span className="font-display text-4xl font-bold tracking-tighter leading-none select-none" style={{ color: 'var(--color-primary-light)' }}>
                    {t(`gov.com_${i}.num`, c.num)}
                  </span>
                  <h3 className="font-display text-lg font-semibold" style={{ color: 'var(--color-ink)' }}>{t(`gov.com_${i}.name`, c.name)}</h3>
                  <p className="text-sm leading-relaxed flex-1" style={{ color: 'var(--color-ink-muted)' }}>{t(`gov.com_${i}.desc`, c.desc)}</p>
                  <p className="text-xs font-medium pt-3 border-t" style={{ color: 'var(--color-ink-subtle)', borderColor: 'var(--color-border)' }}>
                    {t('gov.committee.chair_label', 'Chairperson:')} {t(`gov.com_${i}.chair`, c.chair)}
                  </p>
                </div>
              </Reveal>
            ))}
              ...
              >
                ...
                <div className="flex-1 flex flex-col gap-3">
                  <h3 className="font-display text-lg font-semibold" style={{ color: 'var(--color-ink)' }}>
                    {t('gov.com_extra.title', 'Committee composition')}
                  </h3>
                  <p className="text-sm leading-relaxed" style={{ color: 'var(--color-ink-muted)' }}>
                    {t('gov.com_extra.desc', 'Full membership and chairperson details for all Board committees, as required under SEBI LODR.')}
                  </p>
                </div>
                <p className="text-xs font-semibold pt-3 border-t inline-flex items-center gap-1.5 group-hover:gap-2.5 transition-all" style={{ color: 'var(--color-primary)', borderColor: 'var(--color-border)' }}>
                  {t('gov.com_extra.cta', 'View PDF')} <span aria-hidden>→</span>
                </p>
              </Link>
            </Reveal>
          </div>
      ...
      <section className="py-14 lg:py-12" style={{ background: 'var(--color-bg)' }}>
        <div className="container-wide">
          <Reveal>
            <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--color-primary)' }}>
              {t('gov.resources.label', 'Governance Resources')}
            </p>
            <h2 className="font-display text-3xl lg:text-4xl font-bold leading-snug mb-14 text-balance" style={{ color: 'var(--color-ink)' }}>
              {t('gov.resources.heading', 'Documents & disclosures')}
            </h2>
          </Reveal>
              ...
              <Reveal key={i} delay={i * 50}>
                <Link href={d.href} className="group flex items-start gap-4 p-5 transition-all hover:bg-blue-50/30" style={{ background: 'var(--color-surface)' }}>
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-sm mb-1" style={{ color: 'var(--color-ink)' }}>{t(`gov.doc_${i}.title`, d.title)}</p>
                    <p className="text-xs leading-relaxed" style={{ color: 'var(--color-ink-muted)' }}>{t(`gov.doc_${i}.desc`, d.desc)}</p>
                  </div>
