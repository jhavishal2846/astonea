...
export default async function InvestorInsightsPage() {
  const t = await getPageText('/investor-insights')
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Investor Relations') as string}
        title={t('header.title', 'Investor Insights') as string}
        description={t('header.description', 'All the information, disclosures, and resources an investor in Astonea Labs Limited needs — in one place.') as string}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6">
            {keyFacts.map((f, i) => (
              <div key={i} className="flex flex-col">
                <span className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: 'var(--color-ink-subtle)' }}>{t(`investor.fact_${i}.label`, f.label)}</span>
                <span className="text-sm font-medium" style={{ color: 'var(--color-ink)' }}>{t(`investor.fact_${i}.value`, f.value)}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {quickLinks.map((l, i) => (
              <Reveal key={i} delay={i * 50}>
                <Link href={l.href} className="group flex flex-col p-6 rounded-2xl border h-full transition-all hover:border-blue-200 hover:shadow-md" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                  ...
                  <h3 className="font-semibold text-base mb-2" style={{ color: 'var(--color-ink)' }}>{t(`investor.ql_${i}.title`, l.title)}</h3>
                  <p className="text-xs leading-relaxed flex-1" style={{ color: 'var(--color-ink-muted)' }}>{t(`investor.ql_${i}.desc`, l.desc)}</p>
                  <div className="mt-4 text-xs font-semibold" style={{ color: 'var(--color-primary)' }}>
                    {t('investor.view', 'View →')}
                  </div>
                </Link>
              </Reveal>
            ))}
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <Reveal>
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest mb-4" style={{ color: 'var(--color-primary-light)' }}>
                  {t('investor.contact.label', 'Investor Contact')}
                </p>
                <h2 className="font-display text-3xl font-bold text-white leading-snug mb-5">
                  {t('investor.contact.heading', 'Questions? Reach our Company Secretary.')}
                </h2>
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: 'rgba(255,255,255,0.55)' }}>{t('investor.contact.officer_label', 'Company Secretary & Compliance Officer')}</p>
                    <p className="text-sm text-white">{t('investor.contact.officer_name', 'Mr. Ankit Kapoor')}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: 'rgba(255,255,255,0.55)' }}>{t('investor.contact.email_label', 'Email')}</p>
                    <a href="mailto:cs@astonea.org" className="text-sm hover:underline" style={{ color: 'var(--color-primary-light)' }}>{t('investor.contact.email', 'cs@astonea.org')}</a>
                  </div>
                </div>
              </div>
            </Reveal>
            <Reveal delay={80}>
              <div className="grid grid-cols-2 gap-4">
                {irServices.map((item, i) => (
                  <div key={i} className="p-5 rounded-xl border" style={{ background: 'rgba(255,255,255,0.05)', borderColor: 'rgba(255,255,255,0.1)' }}>
                    <p className="text-sm font-semibold text-white mb-1">{t(`investor.svc_${i}.label`, item.label)}</p>
                    <p className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.72)' }}>{t(`investor.svc_${i}.desc`, item.desc)}</p>
                  </div>
                ))}
              </div>
            </Reveal>
