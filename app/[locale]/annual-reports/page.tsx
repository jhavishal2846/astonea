...
export default async function AnnualReportsPage() {
  ...
  const t = await getPageText('/annual-reports')

  return (
    <div className="flex-1 flex flex-col">
      <PageHeader
        eyebrow={t('header.eyebrow', 'Investor Relations') as string}
        title={t('header.title', 'Annual Reports') as string}
        description={t('header.description', "Astonea Labs Limited's complete annual reports — financial statements, governance, and corporate disclosures.") as string}
        breadcrumb={[
        ...
        <div className="container-wide">
          <Reveal>
            <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--color-primary)' }}>
              {t('doc.archive.eyebrow', 'Reports Archive')}
            </p>
            <h2 className="font-display text-3xl lg:text-4xl font-bold leading-snug mb-14 text-balance" style={{ color: 'var(--color-ink)' }}>
              {t('doc.archive.heading', 'Annual reports for every financial year')}
            </h2>
          </Reveal>

          {reports.length === 0 ? (
            <Reveal>
              <div className="p-6 rounded-xl border max-w-3xl text-sm" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)', color: 'var(--color-ink-muted)' }}>
                {t('doc.empty', 'No annual reports have been published yet.')}
              </div>
            </Reveal>
          ) : (
          ...
          <Reveal delay={200}>
            <div className="mt-10 p-5 rounded-xl border max-w-3xl" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
              <p className="text-xs leading-relaxed" style={{ color: 'var(--color-ink-subtle)' }}>
                {t('doc.disclaimer', 'Annual Reports are filed with BSE in accordance with SEBI LODR Regulations. CIN: L24304CH2017PLC041482 — Astonea Labs Limited, Chandigarh, India. For the most current filings, refer to the exchange portals or contact cs@astonea.org.')}
              </p>
            </div>
          </Reveal>
        </div>
